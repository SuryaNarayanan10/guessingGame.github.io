...
$(document).ready(function () {
        ...
        if (event.which === 13) {
            ...
            if (inputValue.length === 4) {
                if (words.includes(inputValue.toUpperCase())) {
                    ...
                    bullCount = findBull(randomWord, inputValue);
                    cowCount = findCow(randomWord, inputValue) - bullCount;
                    cowString = "cow";
                    bullString = "bull";
                    if (cowCount > 1) cowString = "cows";
                    if (bullCount > 1) bullString = "bulls";

                    totalValue = `- ${cowCount} ${cowString}, ${bullCount} ${bullString}`;

                    const divContent = `<div class="displayValue">
              <span class="clickable">${inputValue[0]}</span>
              <span class="clickable">${inputValue[1]}</span>
              <span class="clickable">${inputValue[2]}</span>
              <span class="clickable">${inputValue[3]}</span>
              <span>${totalValue}</span>
            </div>`;
                    $('#insertHere').prepend(divContent);
                    $('#inputElement').val('');
                    if (bullCount == 4) {
                        $('#displayResult').show();
                        $('#inputElement').hide();
    ...
    $('#insertHere').on('click', '.clickable', function () {
...
function quit() {
    $("#c1").val(randomWordArray[0]);
    $("#c2").val(randomWordArray[1]);
    $("#c3").val(randomWordArray[2]);
    $("#c4").val(randomWordArray[3]);
}

function findBull(randomWord, inputValue) {
    let bullCount = 0;
    let randomWordArray = Array.from(randomWord);
    let inputValueArray = Array.from(inputValue.toUpperCase());
    console.log(randomWordArray, inputValueArray);
    inputValueArray.forEach((value, index) => {
        if (randomWordArray[index] === inputValueArray[index]) {
            ++bullCount;
        }
    });
    return bullCount;
}
function findCow(randomWord, inputValue) {
    let cowCount = 0;
    let randomWordArray = Array.from(randomWord);
    let inputValueArray = Array.from(inputValue.toUpperCase());
    console.log(randomWordArray, inputValueArray);
    inputValueArray.forEach((value, i) => {
        randomWordArray.forEach((value, j) => {
            if (randomWordArray[j] === inputValueArray[i]) {
                ++cowCount;
            }
        });
    });
